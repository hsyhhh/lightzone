...
int thd_s() {
  ...
  printf("core %d -> thread %d\n", ret_val, (int)syscall(SYS_gettid));
  return ret_val;
}
...
void *func(void *p_args) {
    ...
    if(core == 0) {
        start = perf_counter();
        printf("start time: %ld in core 0!\n", start);
    }
